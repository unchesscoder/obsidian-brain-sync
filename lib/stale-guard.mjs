...
export function detectStalePush({ remoteFiles, localFiles, localMtimes, baseline, manifestVault, indexPath }) {
  ...
  for (const rel of remoteFiles || []) {
    ...
    if (!local.has(rel)) {
      ...
      } else if (!remoteRec || !Number.isFinite(remoteRec.mtime)) {
    ...
    if (!remoteRec || !Number.isFinite(remoteRec.mtime)) {
      unsafe.push({ rel, grund: "kein Manifest-Eintrag" });
      continue;
    }
    const localMtime = rel in lm ? lm[rel] : null;
    if (!Number.isFinite(localMtime)) {
      // failed stat leaves the file listed but without a usable mtime -> cannot confirm safety -> block
