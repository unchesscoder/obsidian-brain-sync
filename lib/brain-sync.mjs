...
function syncToRemote(repoDir) {
  ...
  // reset --hard only rewinds TRACKED files. a previous run that wrote into the
  // working tree but never committed (e.g. a push aborted by the secret scan
  // after the mirror was already rewritten) leaves untracked leftovers behind,
  // which would make vault-mirror lie about what the remote actually has. clean
  // those so the working tree truly matches the remote tip, as intended above.
  git(repoDir, ["clean", "-fd", "--", "vault-mirror", "sessions"]);
  return true;
}
...
function secretScan(files, allowlist = []) {
  // allowlist: exact matched-token strings to ignore (known false positives,
  // e.g. placeholder/example tokens). raw values are never logged - only redacted.
  ...
  for (const f of files) {
    ...
    for (let i = 0; i < lines.length; i++) {
      for (const pat of SECRET_PATTERNS) {
        ...
        if (m && !allow.has(m[0])) hits.push({ file: f.rel, line: i + 1, kind: pat.name, sample: redact(m[0]) });
      }
    }
  }
  return hits;
...
function cmdPush(flags) {
  ...
  //    this must also run for --dry-run: the stale-state guard below reads the
  //    remote's file list from this clone, and a dry-run that judges staleness
  //    against a possibly-days-old local clone can reach a different verdict
  //    (and exit code) than the real push it is supposed to preview. it only
  //    touches the engine-managed clone under cfg.repoPath, never the vault, and
  //    never commits or pushes, so it is safe to run unconditionally.
  ...
  // no commit ever landed -> nothing was ever published, so vault-mirror (if it
  // has content at all) is only local leftover from an aborted push, never the
  // remote's state. treat the remote as empty rather than trust that leftover.
  const hasHistory = git(cfg.repoPath, ["rev-parse", "HEAD"]).ok;
  const remoteFiles = hasHistory && fs.existsSync(guardMirror)
    ? walkFiles(guardMirror).map((f) => f.rel.split(path.sep).join("/")).filter((r) => !skip(r))
    : [];
  const localList = listMirrorable(cfg.vaultPath, skip);
  ...
  if (stale.blocked) {
    log(`\nHinweis: --allow-stale gesetzt, ${stale.deletions.length} Loeschungen, ${stale.reversions.length} Rueckdrehungen und ${stale.resurrections.length} Wiederbelebungen werden bewusst gepusht.`);
  }
  ...
  // 5. secret scan over what we are about to push (allowlisted false positives ignored)
  const allowlist = cfg.secretAllowlist || [];
  const allowSecrets = !!flags["allow-secrets"];
  const scanFiles = [
    ...walkFiles(path.join(cfg.repoPath, "vault-mirror")),
    ...walkFiles(path.join(cfg.repoPath, "sessions")),
  ];
  const hits = dry
    ? secretScan([...walkFiles(cfg.vaultPath).filter((f) => !skip(f.rel)), ...walkFiles(cfg.sessionsRoot)], allowlist)
    : secretScan(scanFiles, allowlist);
  if (hits.length) {
    log(`\nSECRET-WARNUNG: ${hits.length} moegliche Geheimnisse gefunden:`);
  ...
  if (dry) {
    ...
    if (hits.length && !allowSecrets) log("Hinweis: Diese Funde wuerden den echten Push BLOCKIEREN (ausser mit --allow-secrets).");
    if (hits.length) process.exitCode = 3;
    return;
  }

  // 5b. block the push if real secrets are present (the safe default). nothing is
  //     committed or pushed. resolve by cleaning the file, allowlisting a known false
  //     positive (config 'secretAllowlist'), or pushing on purpose with --allow-secrets.
  if (hits.length && !allowSecrets) {
    log(`\nPush ABGEBROCHEN: ${hits.length} moegliche Geheimnisse wuerden hochgeladen - nichts wurde committet oder gepusht.`);
    log("Optionen:");
    log("  - betroffene Datei bereinigen, dann erneut pushen");
    log("  - bekannten Fehlalarm in config 'secretAllowlist' eintragen (exakter Treffer-String)");
    log("  - bewusst trotzdem pushen: --allow-secrets");
    process.exitCode = 4;
    return;
  }
