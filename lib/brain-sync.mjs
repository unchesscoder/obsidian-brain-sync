...
function emptyManifest() {
  return { version: 1, sessions: {}, history: [] };
}
...
function ensureRepoClone(cfg) {
  ...
  if (fs.existsSync(path.join(cfg.repoPath, ".git"))) return;
...
function cmdPush(flags) {
  ...
  // 1. pull remote first so we don't clobber another machine's newer state
  if (!dry) {
    const pr = git(cfg.repoPath, ["pull", "--no-rebase"]);
    if (!pr.ok && /conflict/i.test(pr.err + pr.out)) {
      die("Merge-Konflikt im Sync-Repo. Bitte manuell aufloesen in:\n  " + cfg.repoPath);
    }
  }
...
function cmdPull(flags) {
  ...
  if (!dry) {
    const pr = git(cfg.repoPath, ["pull"]);
    if (!pr.ok && /conflict/i.test(pr.err + pr.out))
      die("Merge-Konflikt im Sync-Repo. Bitte manuell aufloesen in:\n  " + cfg.repoPath);
  }
  ...
  // 2. mirror -> vault (additive overwrite; never deletes local files)
  const mirrorDir = path.join(cfg.repoPath, "vault-mirror");
  const skip = makeSkipper(cfg.excludes);
  const stats = { files: 0, nestedSkipped: [] };
  if (fs.existsSync(mirrorDir)) {
    if (!dry) copyTree(mirrorDir, cfg.vaultPath, skip, stats);
    else stats.files = walkFiles(mirrorDir).filter((f) => !skip(f.rel)).length;
  }
  log(`Vault aktualisiert: ${stats.files} Dateien${dry ? " (dry)" : ""}`);
  ...
  if (warnings.length) { log("\nRemap-Hinweise:"); warnings.forEach((w) => log("  " + w)); }
